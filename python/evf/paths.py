...
def tetra3rs_database_path(filename: str = "tetra3rs_gaia.bin") -> Path:
    """Path to a prebuilt tetra3rs SolverDatabase .bin.

    Generated from the bundled gaia-catalog package and shipped in
    data/ for all builds. ``filename`` comes from a built-in camera
    profile, not user input.
    """
    if _BUNDLE_MODE:
        return _RESOURCES / filename
    if _LINUX_RELEASE or _WINDOWS_RELEASE:
        return _RELEASE_ROOT / "data" / filename
